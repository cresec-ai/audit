...
- **Claude Code is the one surface where a third party gets pre-execution
  control.** Its PreToolUse/PostToolUse hooks fire for every connector tool
...
| Claude Code cloud session | Connectors are `type: http` MCP servers in `/tmp/mcp-config-<session>.json`, pointing at `api.anthropic.com/v2/ccr-sessions/<session>/mcp?mcp_url=<vendor>` with `X-MCP-Server-ID`, `X-MCP-Server-Origin` and `X-Session-UUID` headers; the relay forwards to the vendor. [observed] | PreToolUse/PostToolUse hooks from the repository's `.claude/settings.json` fire with full `tool_input`/`tool_response`; the session transcript JSONL holds every `tool_use`/`tool_result`. [observed] | Hooks can deny or rewrite. `mcp-recorder hook` records and enforces. |
