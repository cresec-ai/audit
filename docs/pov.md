...
- *"If the gateway is down the tool goes read-only."* — no, and by design.
  When the control plane is unreachable the MCP gateway refuses every call
  that needs it (`control_plane_unavailable`), quickly and as a retryable
...
4. **Degrade mode, MCP leg** — fail closed, fast and retryable when the
   control plane is unreachable, never a credential or cached-token
