...
| `mcp-recorder sessions [--data-dir D] [--store sqlite\|jsonl] [--json]` | List recorded sessions: first server, number of distinct servers (`SERVERS` / `server_count`), identity, event/tool-call/error counts. Tool calls are counted per call, so a hook-captured call (a `pre` + `post` event pair) counts once. |
