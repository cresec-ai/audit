...
| `mcp-recorder sessions [--data-dir D] [--store sqlite\|jsonl] [--json]` | List recorded sessions: first server, identity, event/tool-call/error counts, the number of distinct servers the session's tool calls went to (`SERVERS`; `server_count` in `--json`, an additive key), and — last column — `LAST_EVENT`, the timestamp of the session's last event (`last_event_at`, additive), which is the instant every count in the row runs through. Tool calls are counted per call, so a hook-captured call (a `pre` + `post` event pair) counts once. `ENDED` shows an end time only when the session's `session_end` really is its last event: a session resumed under the same id keeps recording after its `session_end`, and reads `(reopened)` rather than a superseded end time (`--json` still carries that `ended_at`). `--json` is the stable machine interface; the table is for reading. |
