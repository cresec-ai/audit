...
function inflateZipEntry(buf, localHeaderOffset, method, compressedSize, name, maxEntryBytes) {
    ...
    if (method === 8) {
        try {
            ...
            if (code === 'ERR_BUFFER_TOO_LARGE' || /larger than/i.test(cause.message)) {
                err(`${name} inflates past the ${maxEntryBytes / (1024 * 1024)} MiB limit of verify --bundle on a .zip; ` +
                    'extract the archive and run verify --bundle on the directory (or node verify.cjs inside it)');
            }
            err(`malformed ZIP: cannot inflate ${name}: ${cause.message}`);
        }
    }
 ...
 */
export function readZipEntries(buf, wanted, opts = {}) {
    const maxEntryBytes = opts.maxEntryBytes ?? ZIP_MAX_ENTRY_BYTES;
    const eocd = findZipEndOfCentralDirectory(buf);
    if (eocd === -1)
        err('not a valid ZIP file (no end-of-central-directory record found)');
    ...
    for (let i = 0; i < totalEntries; i++) {
        ...
        if (!wantedSet.has(name))
            continue;
        if (found.has(name))
            err(`malformed ZIP: duplicate entry ${name}`);
