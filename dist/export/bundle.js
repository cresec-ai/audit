 ...
 */
function selectRecords(store, sessionId) {
...
function readmeText(manifest) {
    const scope = manifest.session_id === undefined
...
export async function exportBundle(opts) {
    ...
    if (records.length === 0) {
        throw new Error(opts.sessionId === undefined
    ...
    if (opts.sessionId !== undefined)
        manifest.session_id = opts.sessionId;
    const entries = [
        {
 ...
 */
const VERIFY_CJS = String.raw `#!/usr/bin/env node
/*
