 ...
 * "no match".
 */
export function detectWsl(env, readFile, platform = process.platform, exists = nodeExistsSync) {
    if (platform !== 'linux')
        return { inWsl: false };
...
const defaultSpawnSync = (command, args, options) => {
...
function windowsUserProfileVia(spawn, exists) {
    for (const cmd of CMD_EXE_CANDIDATES) {
        if (cmd.startsWith('/') && !exists(cmd))
            continue;
        ...
        if (line.length === 0 || line.includes('%USERPROFILE%'))
            continue; // didn't expand
        return line;
    }
    return undefined;
}
...
function manualWinToWslPath(winPath) {
...
function convertWindowsPath(winPath, spawn) {
...
function globWindowsUsersDir(readdir, exists) {
    if (!exists(WINDOWS_USERS_DIR))
        return [];
    let entries;
    try {
 ...
 */
export function windowsHomeCandidates(deps = {}) {
 ...
 * filesystem.
 */
function msixClaudeDesktopCandidates(home, readdirFn) {
 ...
 */
export function windowsClientConfigCandidates(client, homes, readdirFn = nodeReaddirSync) {
    switch (client) {
        case 'claude-desktop':
 ...
 */
export function chooseWrapper(explicit, inWsl, configPath) {
    if (explicit !== undefined)
        return explicit;
    if (inWsl && isWindowsMountPath(configPath))
        return 'wsl';
    return 'local';
}
