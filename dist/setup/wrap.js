 ...
 * wrap shape.
 */
export function recorderPrefixArgs(entry) {
    ...
    if (sepIdx === -1)
        return undefined;
    return args.slice(0, sepIdx).filter((a) => typeof a === 'string');
}
/**
 ...
 */
export function recorderPolicyArg(entry) {
 ...
 */
export function parseBridgeSpecs(values) {
    const specs = [];
    for (const value of values) {
        for (const part of value.split(',')) {
            ...
            if (!BRIDGE_NAME_RE.test(name)) {
                throw new Error(`setup: invalid --bridge name '${name}' (letters, digits, '_', '.', '-' only)`);
            }
            let parsed;
            try {
            ...
            if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
                throw new Error(`setup: invalid --bridge URL '${urlStr}' for '${name}' (must be http:// or https://)`);
            }
 ...
 * other way.
 */
export function bridgeEntry(url) {
 ...
 */
export function isSameBridgeEntry(entry, url) {
