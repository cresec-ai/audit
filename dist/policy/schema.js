...
export const POLICY_SCHEMA = {
    ...
    $defs: {
        ...
        credential: {
            ...
            additionalProperties: false,
            required: ['id', 'use'],
            description: 'One credential. Exactly one of "source" (resolved on this machine) or "broker" (resolved by the Cresec control plane per user) is required.',
            properties: {
                id: { $ref: '#/$defs/identifier', description: 'Unique within the section. Recorded on every decision; never the credential value.' },
                provider: {
                    $ref: '#/$defs/identifier',
                    description: 'Informational, e.g. "github". Recorded on the decision. For a "broker" credential it is the connector sent to the control plane and is required; validation then holds it to the control plane\'s closed set (salesforce, gmail, workspace, slack, outlook) — a conditional this schema\'s keyword subset cannot express.',
                },
                scopes: {
                    type: 'array',
                ...
                synthetic_env: {
                    ...
                    description: 'Environment variable holding the synthetic bound to this credential. Defaults to MCP_RECORDER_SYNTHETIC_<ID>. The synthetic is deliberately not in the policy: a policy file is shareable evidence of the rules, and a file of raw synthetics is a file of values redeemable on this machine.',
                },
                on_unresolved: {
                    type: 'string',
                    enum: ['deny'],
        ...
        credentialUse: {
            ...
            properties: {
                ...
                action_class: {
                    ...
                    description: 'The control plane\'s action class for this site (user-token.md). Sent as action_class on the per-user token request of a "broker" credential; informational otherwise. Default "write", the class that always needs a grant.',
                },
                method: {
                    type: 'string',
                    pattern: '^[A-Za-z]{1,16}$',
