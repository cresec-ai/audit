...
export function checkPortableText(text) {
    const m = LONE_SURROGATE.exec(text);
    if (m === null)
        return undefined;
    const code = m[0].charCodeAt(0).toString(16).padStart(4, '0');
    return (`the unpaired surrogate \\u${code} cannot be represented in a Rego string ` +
...
export function checkGlob(glob) {
    ...
    if (glob.includes('?')) {
        return ('the ? wildcard is not supported in policy.yaml v1 (use * or **): ' +
 ...
 */
const PORTABLE_LETTER_ESCAPES = new Set(['d', 'D', 'w', 'W', 'b', 'B', 'n', 'r', 't', 'f', 'v']);
 ...
 */
function checkEscape(pattern, i, inClass) {
    ...
    if (next === 'k')
        return 'named backreference "\\k<name>" is not supported (RE2 subset)';
    if (next === 'x') {
        if (pattern[i + 2] === '{')
    ...
    if ((next === 'b' || next === 'B') && inClass) {
        return `escape "\\${next}" inside a character class is not supported: RE2 rejects it and JavaScript reads it as ${next === 'b' ? 'a backspace' : 'a literal "B"'}`;
    }
    if (PORTABLE_LETTER_ESCAPES.has(next))
        return undefined;
...
export function checkRe2Subset(pattern) {
    ...
    try {
        ...
        return ('pattern cannot be matched in Unicode mode, which the local engine needs so that it counts characters' +
...
const REPEAT_AT = /^\{(\d+)(?:,(\d*))?\}/;
function checkRepeat(pattern, at) {
    const m = REPEAT_AT.exec(pattern.slice(at));
    if (m === null)
        return undefined; // a literal "{", which both engines accept
    const counts = [m[1], m[2]].filter((c) => c !== undefined && c.length > 0);
    for (const c of counts) {
        if (Number(c) > RE2_MAX_REPEAT) {
            return (`repeat count ${c} is above RE2's limit of ${RE2_MAX_REPEAT}: the compiled policy would fail to` +
 ...
 */
function repeatBudgetProblem(pattern, from, to, budget, parens) {
    const tooBig = (charge, left) => `nested repeat counts multiply to more than RE2's limit of ${RE2_MAX_REPEAT} ` +
    ...
    for (let i = from; i < to; i++) {
        ...
        if (ch === '(') {
            ...
            if (close === undefined || close >= to)
                continue; // unbalanced: `new RegExp` rejects it
...
function checkGroupPrefix(pattern, at, groupNames) {
    ...
    if (first === '<') {
        ...
        if (close < 0)
            return undefined; // malformed: `new RegExp` below reports it
        const name = pattern.slice(at + 1, close);
        if (!RE2_GROUP_NAME.test(name)) {
            return (`capture-group name "${name}" is not valid in RE2, which allows only letters, digits and "_":` +
...
function checkDuplicateIds(rules, base, errors, noun = 'rule') {
    const seen = new Map();
...
function checkReason(reason, path, errors) {
    if (typeof reason !== 'string')
        return; // the schema already reported the type
    const why = checkPortableText(reason);
    if (why !== undefined)
        errors.push({ path, message: why, keyword: 'text' });
}
...
const GITHUB_PERMISSION_LEVELS = new Set(['read', 'write', 'admin']);
function checkAbsolutePath(value, path, errors) {
    if (typeof value !== 'string')
        return; // the schema already reported the type
...
function checkDotPath(value, path, errors) {
    if (typeof value !== 'string')
        return;
...
function checkCredentialSource(source, base, errors) {
    switch (source.type) {
        case 'file':
            checkAbsolutePath(source.path, `${base}/path`, errors);
            if (source.field !== undefined)
                checkDotPath(source.field, `${base}/field`, errors);
            return;
        case 'exec':
        ...
        case 'github-app': {
            ...
            if (source.private_key_file !== undefined)
                checkAbsolutePath(source.private_key_file, `${base}/private_key_file`, errors);
...
function checkCredentialUse(use, base, errors) {
    checkReason(use.reason, `${base}/reason`, errors);
    if (use.server !== undefined)
        checkGlobField(use.server, `${base}/server`, errors);
...
function checkControlPlaneUrl(value, path, errors) {
    if (typeof value !== 'string')
        return;
    let url;
    try {
 ...
 */
export const CONTROL_PLANE_CONNECTORS = ['salesforce', 'gmail', 'workspace', 'slack', 'outlook'];
function checkRemoteBroker(credential, base, errors) {
    ...
    if (credential.provider === undefined) {
        ...
        errors.push({
            path: `${base}/provider`,
            message: `"${credential.provider}" is not a connector the control plane knows; a "broker" credential's provider must be one of ` +
...
function checkCredential(credential, i, errors) {
    ...
    if (credential.source !== undefined)
        checkCredentialSource(credential.source, `${base}/source`, errors);
    checkRemoteBroker(credential, base, errors);
    if (credential.use === undefined)
        return;
