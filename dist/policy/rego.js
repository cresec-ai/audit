...
export const CREDENTIALS_REGO_PATH = 'cresec/credentials/broker.rego';
...
export const CREDENTIALS_ROOT = 'cresec/credentials';
...
export function bundleRoots(policy) {
    const roots = [MCP_ROOT];
    if (policy.credentials !== undefined)
        roots.push(CREDENTIALS_ROOT);
    if (policy.egress !== undefined)
        roots.push(EGRESS_ROOT);
    return roots;
}
/** Canonical write order for bundle files. */
export const BUNDLE_FILE_ORDER = [MANIFEST_PATH, MCP_REGO_PATH, CREDENTIALS_REGO_PATH, EGRESS_REGO_PATH];
...
export const DECISION_SHAPE = '{"allow": bool, "action": "allow"|"hold"|"deny", "rule_id": "...", "reason": "...", "matched": bool, "deny_reason": "..."}';
/**
...
/** Every swap site of every credential, in file order. */
function credentialRules(credentials) {
    const out = [];
    for (const credential of credentials) {
        for (const use of credential.use) {
            const rule = { id: use.id, action: use.action, credentialId: credential.id, use };
            if (use.reason !== undefined)
                rule.reason = use.reason;
            out.push(rule);
        }
    }
    return out;
...
/**
 * One swap site as Rego. The credential is matched by id — the control plane
 * resolves the synthetic to it before asking — and then the same four
 * predicates the gateway checks locally.
 *
 * `input.host_source` is a condition, not decoration. The three host forms
 * are not interchangeable: only `argument` constrains where the credential
 * actually goes, `declared` is the operator's assertion about a server that
 * takes no destination argument, and `server_name` is no destination at all.
 * Requiring the data plane to say which one it filled `host` from means a
 * site declared with a checked destination cannot be satisfied by a request
 * that merely put the server name there — and the decision log distinguishes
 * "host was checked" from "host was the server name" without anybody having
 * to reconstruct it.
 *
 * For a `server_name` site the host is matched against the site's SERVER
 * globs (delimiter "/"), because in that form the two are the same string.
 */
function credentialRuleBody(rule) {
    const use = rule.use;
    const lines = [`input.credential == ${q(rule.credentialId)}`];
    lines.push(...globLines(use.server, '/', 'input.server', 's'));
    lines.push(...globLines(use.tool, '/', 'input.tool', 't'));
    lines.push(`input.host_source == ${q(use.host.from)}`);
    if (use.host.from === 'argument') {
        lines.push(...globLines(use.host.allow, '.', 'input.host', 'h'));
    }
    else if (use.host.from === 'declared') {
        lines.push(`input.host == ${q(use.host.host)}`);
    }
    else {
        lines.push(...globLines(use.server, '/', 'input.host', 'h'));
    }
    if (use.path.from === 'argument') {
        lines.push(...globLines(use.path.allow, '/', 'input.path_template', 'p'));
    }
    else {
        // The default binding says path_template IS the tool name (the contract's
        // "MCP: the tool name"), so a request that filled it with anything else
        // has not been described by this policy and does not match.
        lines.push('input.path_template == input.tool');
    }
    return lines;
}
/**
 ...
 */
export function renderCredentialsModule(policy, opts) {
    const credentials = policy.credentials;
    if (credentials === undefined)
        throw new TypeError('policy has no credentials section');
    return renderModule({
        pkg: 'cresec.credentials',
        inputShape: '{"credential": "...", "server": "...", "tool": "...", "host": "...", "host_source": "argument"|"declared"|"server_name", "path_template": "..."}',
        // Not the section's choice: an undeclared use of a credential is a deny,
        // always. The author picks which sites exist, never what happens off them.
        defaultAction: 'deny',
        rules: credentialRules(credentials),
        body: credentialRuleBody,
        notes: [
            '',
...
export function compileToRego(policy, opts) {
    ...
    if (policy.credentials !== undefined)
        files[CREDENTIALS_REGO_PATH] = renderCredentialsModule(policy, opts);
    if (policy.egress !== undefined)
