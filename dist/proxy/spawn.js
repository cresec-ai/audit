 ...
 * here.
 */
export function resolveCommand(command, env, platform, fs = defaultFs) {
    ...
    const candidates = (base) => (hasKnownExt ? [base] : exts.map((ext) => base + ext));
 ...
 */
export function escapeCmdArg(arg, doubleEscapeMetaChars) {
 ...
 */
function escapeBackslashRuns(arg) {
    let out = '';
    let run = 0;
 ...
 */
export function planSpawn(argv, env, platform, fs = defaultFs) {
    ...
    if (broken !== -1) {
        throw new Error(`refusing to launch ${command} through cmd.exe: argument ${broken} contains a line break ` +
 ...
 */
export function spawnWrapped(argv, env, plan = planSpawn(argv, env, process.platform)) {
 ...
 */
export function withNodeDirOnPath(env, execPath, platform = process.platform) {
    ...
    const alreadyPresent = segments.some((seg) => platform === 'win32' ? seg.toLowerCase() === dir.toLowerCase() : seg === dir);
    if (alreadyPresent || dir.length === 0)
        return env;
    ...
    const firstWindowsMount = platform === 'win32' ? -1 : segments.findIndex((seg) => /^\/mnt\/[a-zA-Z](\/|$)/.test(seg));
    const next = firstWindowsMount === -1
        ? [...segments, dir]
 ...
 */
export function terminateChild(child, signal, platform = process.platform, taskkill = defaultTaskkill) {
    if (platform === 'win32' && child.pid !== undefined && taskkill(child.pid))
        return;
    try {
        child.kill(signal);
    }
