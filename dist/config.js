...
export function ensureDataDir(dataDir) {
    ...
    try {
        ...
        if ((mode & 0o077) !== 0) {
            process.stderr.write(`[mcp-recorder] warning: data dir ${dataDir} is group/world accessible (mode ${mode.toString(8)}); ` +
...
function resolveCommon(opts) {
    ...
    const dataDirRaw = pick(flags['data-dir'], env[ENV.DATA_DIR]) ?? join(homedir(), '.mcp-recorder');
    const out = {
    ...
    if (identityLabel !== undefined)
        out.identityLabel = identityLabel;
    return out;
}
export function resolveConfig(opts) {
    ...
    if (storeRaw !== undefined) {
        if (storeRaw !== 'sqlite' && storeRaw !== 'jsonl') {
            throw new Error(`invalid store backend '${storeRaw}' (expected 'sqlite' or 'jsonl')`);
        }
        storeBackend = storeRaw;
    }
    const redactRaw = pick(flags['redact'], env[ENV.REDACT]) ?? 'allowlist';
    if (redactRaw !== 'allowlist' && redactRaw !== 'off') {
        throw new Error(`invalid redact mode '${redactRaw}' (expected 'allowlist' or 'off')`);
    }
    const config = {
    ...
    if (storeBackend !== undefined)
        config.storeBackend = storeBackend;
    if (common.serverName !== undefined)
        config.serverName = common.serverName;
    if (common.identityLabel !== undefined)
        config.identityLabel = common.identityLabel;
    return config;
}
/** Same resolution as `resolveConfig`, but fail-open: never throws. */
export function resolveConfigLenient(opts) {
    const { flags, env } = opts;
    const common = resolveCommon(opts);
    const warnings = [];
    const storeRaw = pick(flags['store'], env[ENV.STORE]);
    ...
    if (storeRaw !== undefined) {
        if (storeRaw !== 'sqlite' && storeRaw !== 'jsonl') {
            warnings.push(`invalid store backend '${storeRaw}' (expected 'sqlite' or 'jsonl'); using automatic selection`);
        }
        else {
            storeBackend = storeRaw;
        }
    ...
    if (redactRaw === 'allowlist' || redactRaw === 'off') {
        ...
        warnings.push(`invalid redact mode '${redactRaw}' (expected 'allowlist' or 'off'); using 'allowlist'`);
