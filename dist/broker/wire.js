 ...
 */
export function registerPolicyEnvSecrets(credentials, env) {
    const refused = [];
    for (const cred of credentials) {
        for (const name of sourceEnvVars(cred)) {
            ...
            if (value === undefined || value === '')
                continue;
            if (!registerBrokeredSecret(value))
                refused.push(name);
        }
    }
    return refused;
    ...
    switch (src.type) {
        ...
        case 'file':
            if (src.field !== undefined) {
                throw new CredentialWiringError(`${at}: source.field is not supported by the local file source — it reads the whole file. ` +
        ...
        case 'github-app': {
            ...
            const key = src.private_key_file !== undefined
        ...
        case 'aws-sts': {
            if (src.external_id !== undefined) {
                throw new CredentialWiringError(`${at}: aws-sts external_id is not supported by the local broker — it is an AssumeRole ` +
 ...
 */
export function credentialSwapFromPolicy(opts) {
    ...
    if (refused.length > 0) {
        throw new CredentialWiringError(`the brokered-secret exclusion set is full; cannot guarantee ${refused.join(', ')} stays out of the ` +
    ...
    for (const cred of credentials) {
        ...
        entries.push({
            ...
            sites: cred.use.map((use) => ({
                ...
                ...(use.path.from === 'argument' ? { path_from: { kind: 'argument', arg: use.path.arg } } : {}),
            })),
    ...
    for (const id of unbound) {
        warn(`credentials: "${id}" declares swap sites but ${envVarFor(credentials, id)} is not set — ` +
