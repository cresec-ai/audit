...
function loadJsonlFile(path) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            if (isLastContent) {
                process.stderr.write(`[mcp-recorder] ignoring trailing partial line ${i + 1} in ${path}\n`);
                break;
            }
            throw new Error(`mcp-recorder: corrupt JSONL line ${i + 1} in ${path}: ${err.message}`);
        }
    }
    return out;
}
 ...
 */
function isLineStart(path, offset) {
    if (offset <= 0)
        return true;
    let fd;
    try {
 ...
 */
function readJsonlFrom(path, fromOffset) {
    let fd;
    try {
        fd = openSync(path, 'r');
    }
    ...
    try {
        ...
        for (let i = 0; i < bytes.length; i++) {
            ...
            try {
                ...
                throw new Error(`mcp-recorder: corrupt JSONL record at byte offset ${fromOffset + lineStart - line.length - 1} in ${path}: ${err.message}`);
            }
        }
 ...
 */
function syncJsonlArray(path, current, loadedSize) {
    const size = sizeOf(path);
    if (size === loadedSize)
        return { items: current, loadedSize };
    if (size < loadedSize || !isLineStart(path, loadedSize)) {
        return { items: loadJsonlFile(path), loadedSize: sizeOf(path) };
    }
    const { items: added, nextOffset } = readJsonlFrom(path, loadedSize);
    return {
...
function repairTornTail(path) {
    ...
    try {
        ...
        process.stderr.write(`[mcp-recorder] discarded a torn trailing line (${size - cut} bytes) in ${path} left by an interrupted write\n`);
        return size - cut;
    }
    finally {
        closeSync(fd);
...
function acquireLock(lockDir) {
    ...
    for (;;) {
        try {
            ...
            const backoff = Math.min(LOCK_RETRY_MAX_MS, LOCK_RETRY_BASE_MS * 2 ** attempt) + Math.random() * 5;
            attempt++;
            sleepSync(backoff);
        }
    }
...
export class JsonlStore {
    ...
    constructor(dataDir) {
        ...
        this.sigs = loadJsonlFile(this.sigsPath);
    ...
    readHeadFromDisk() {
        ...
        try {
            ...
            const records = loadJsonlFile(this.path);
    ...
    *iterate(opts = {}) {
        ...
        for (const record of this.records) {
            if (opts.fromSeq !== undefined && record.seq < opts.fromSeq)
                continue;
            if (opts.toSeq !== undefined && record.seq > opts.toSeq)
                continue;
            if (opts.sessionId !== undefined && record.event.session_id !== opts.sessionId)
                continue;
            yield record;
        }
    }
