...
function malformedFields(record) {
    if (typeof record !== 'object' || record === null)
        return ['record'];
    const bad = [];
    const r = record;
    if (typeof r.seq !== 'number' || !Number.isInteger(r.seq) || r.seq < 1)
        bad.push('seq');
    if (typeof r.prev_hash !== 'string' || !HEX64.test(r.prev_hash))
        bad.push('prev_hash');
    if (typeof r.hash !== 'string' || !HEX64.test(r.hash))
        bad.push('hash');
    if (typeof r.event !== 'object' || r.event === null || Array.isArray(r.event))
        bad.push('event');
    return bad;
}
/** Shape check before attempting cryptographic verification. */
function malformedSignature(sig) {
    if (typeof sig !== 'object' || sig === null)
        return 'signature entry is not an object';
    ...
    if (sig.algo !== 'ed25519')
        return `unsupported algo ${String(sig.algo)}`;
...
export async function verifyRecords(records, signatures, opts = {}) {
    ...
    for (const record of records) {
        ...
        checked++;
        if (record.event.kind === 'session_end')
            sessionEndSeqs.push(record.seq);
        if (prev === undefined) {
        ...
        try {
            ...
            problems.push({
                type: 'malformed_record',
                seq: record.seq,
                detail: `event cannot be canonicalized: ${err.message}`,
            });
        }
        prev = record;
    }
    const head = prev === undefined ? { seq: 0, hash: baseHash } : { seq: prev.seq, hash: prev.hash };
    // ------------------------------ signatures ------------------------------
    let verified;
    ...
    for (const sig of signatures) {
        ...
        if (shapeIssue !== undefined) {
            problems.push({
                ...
                seq: typeof sig?.seq === 'number' ? sig.seq : 0,
                detail: `malformed signature: ${shapeIssue}`,
            });
            continue;
        }
        if (opts.expectedPublicKeyHex !== undefined &&
        ...
        try {
            cryptoValid = ed.verify(ed.etc.hexToBytes(sig.signature), signedPayload(sig.seq, sig.chain_hash), ed.etc.hexToBytes(sig.public_key));
        }
        catch {
            cryptoValid = false;
        }
        ...
        if (verified === undefined || sig.seq >= verified.seq)
            verified = sig;
    }
    ...
    if (head.seq > 0 && !sawChainMismatchOrTruncation) {
        ...
        const warnOpt = opts.allowUnsigned === true ? { warning: true } : {};
        if (verified === undefined) {
            problems.push({
                type: 'no_valid_signature',
                seq: head.seq,
                detail: `the store carries no valid signature: ${checked} event(s) recorded, but no head ` +
            ...
            if (tailHasSessionEnd) {
                problems.push({
                    ...
                    seq: head.seq,
                    detail: 'the chain includes a session_end event with no valid signature covering it — ' +
            ...
            if (tailHasSessionEnd) {
                problems.push({
                    type: 'unsigned_session_end',
                    seq: head.seq,
                    detail: `events after seq ${verified.seq} (through ${head.seq}) are unsigned and include a ` +
    ...
    if (verified !== undefined)
        result.verified_signature = verified;
    return result;
}
/** Verify everything an evidence store holds (whole chain + all signatures). */
export async function verifyStore(store, opts = {}) {
