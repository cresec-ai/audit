 ...
 */
export function friendlyName(configKey, host) {
    if (!looksLikeUuid(configKey))
        return configKey;
    if (host === undefined)
        return undefined;
 ...
 */
export function toolSpellings(input) {
    ...
    for (const c of candidates) {
        if (c.segment === undefined || c.segment === '')
            continue;
 ...
 */
export const PROBE_CONNECTOR_TOOLS = [
    'clickup_delete_task',
    'clickup_create_task',
    'trash_message',
    'send_message',
 ...
 */
export function anchoredSegment(source) {
    const at = source.indexOf('mcp__');
    if (at === -1)
        return undefined; // not a rule about the mcp__ namespace
    const rest = source.slice(at + 'mcp__'.length);
    const end = rest.indexOf('__');
    if (end === -1)
        return undefined; // no segment/tool split to anchor
...
//# sourceMappingURL=spellings.js.map