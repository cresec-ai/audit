 ...
 */
export function cliEntryPoint(moduleUrl = import.meta.url) {
 ...
 */
export function ensureShipper(opts) {
    try {
        if (opts.config.disabled)
            return false;
        if (shipperLooksAlive(opts.config.dataDir))
            return false;
        ...
        if (opts.config.storeBackend !== undefined)
            args.push('--store', opts.config.storeBackend);
        const childEnv = { ...env, [ENV.SINK]: opts.sink.url };
        ...
        const inherited = (env[ENV.SINK_TOKEN] ?? '') !== '' || (env[ENV.SINK_TOKEN_FILE] ?? '') !== '';
        if (!inherited && opts.sink.token !== undefined)
            childEnv[ENV.SINK_TOKEN] = opts.sink.token;
