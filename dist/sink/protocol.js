 ...
 */
export function chainIdFromGenesisRecord(genesis) {
    if (genesis.seq !== 1) {
 ...
 */
export function sinkSignedPayload(chainId, fromSeq, toSeq, contentSha256Hex) {
    return new TextEncoder().encode(`${SINK_SIGNATURE_DOMAIN}\n${chainId}\n${fromSeq}\n${toSeq}\n${contentSha256Hex}`);
}
/** Path of the only write endpoint. */
export function recordsPath(chainId) {
    return `/v1/chains/${encodeURIComponent(chainId)}/records`;
}
/** Path of the resume oracle. */
export function cursorPath(chainId) {
...
export function parseCursor(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return undefined;
    const raw = value;
    const body = typeof raw.cursor === 'object' && raw.cursor !== null && !Array.isArray(raw.cursor)
        ? raw.cursor
        : raw;
    const nextSeq = body.next_seq;
    if (typeof nextSeq !== 'number' || !Number.isInteger(nextSeq) || nextSeq < 1)
        return undefined;
    if (!isHex64(body.head_hash))
        return undefined;
    if (!isHex64(body.chain_id))
        return undefined;
    if (!isHex64(body.key))
        return undefined;
    const cursor = {
    ...
    if (typeof body.attested_at === 'string')
        cursor.attested_at = body.attested_at;
...
export function backoffMs(attempt, random, base = BACKOFF_BASE_MS, cap = BACKOFF_CAP_MS) {
