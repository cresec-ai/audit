 ...
 * reads on its own.
 */
export function normalizeSinkUrl(raw) {
    let parsed;
    try {
        parsed = new URL(raw);
    }
    ...
    if (parsed.username !== '' || parsed.password !== '') {
        return {
            error: `${ENV.SINK} must not carry userinfo (it leaks into process listings and logs) — ` +
    ...
    if (parsed.protocol === 'http:') {
        if (!isLoopbackHost(parsed.hostname)) {
            return {
                error: `refusing plain http sink '${parsed.protocol}//${parsed.host}' — ` +
 ...
 */
export function resolveSinkConfig(opts) {
    ...
    if (inlineToken !== undefined) {
        ...
        if (tokenFile !== undefined) {
            warnings.push(`both ${ENV.SINK_TOKEN} and ${ENV.SINK_TOKEN_FILE} are set; using the inline token`);
        }
    }
        ...
        warnings.push(`${ENV.SINK} is set but no token (${ENV.SINK_TOKEN} / ${ENV.SINK_TOKEN_FILE}); ` +
