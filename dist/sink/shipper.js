 ...
 */
export function collectBatch(store, fromSeq, maxRecords, maxBytes) {
    const records = [];
    let bytes = 0;
    for (const record of store.iterate({ fromSeq })) {
        ...
        if (records.length > 0 && record.seq !== records[records.length - 1].seq + 1)
            break;
        const size = JSON.stringify(record).length;
        if (records.length > 0 && (records.length >= maxRecords || bytes + size > maxBytes))
            break;
        records.push(record);
        bytes += size;
...
export function signaturesInRange(store, fromSeq, toSeq) {
...
function recordAt(store, seq) {
    for (const record of store.iterate({ fromSeq: seq, toSeq: seq }))
        return record;
    return undefined;
}
/**
 ...
 */
export async function runShipper(opts) {
    const now = opts.now ?? Date.now;
    const sleep = opts.sleep ?? defaultSleep;
    const random = opts.random ?? Math.random;
    const pollMs = opts.pollIntervalMs ?? DEFAULT_POLL_MS;
    const idleExitMs = opts.idleExitMs ?? DEFAULT_IDLE_EXIT_MS;
    const drainDeadline = opts.drain === true && opts.drainTimeoutMs !== undefined
     ...
     */
    const isParked = (s) => s === 'forked' || s === 'stalled' || s === 'refused';
    const isHealthy = (s) => s === 'idle' || s === 'shipping' || s === 'retrying';
    ...
    const writeStatus = (localHeadSeq) => {
        ...
        if (cursor?.attested_seq !== undefined)
            status.attested_seq = cursor.attested_seq;
        if (lastError !== undefined)
            status.last_error = lastError;
        if (lastErrorAt !== undefined)
            status.last_error_at = lastErrorAt;
        if (lastSuccessAt !== undefined)
            status.last_success_at = lastSuccessAt;
    ...
    const applyCursor = (next) => {
        ...
        if (next.max_records !== undefined)
            maxRecords = Math.min(SENDER_MAX_RECORDS, next.max_records);
        if (next.max_bytes !== undefined)
            maxBytes = Math.min(SENDER_MAX_BYTES, next.max_bytes);
        if (next.heartbeat_interval_s !== undefined)
            heartbeatMs = next.heartbeat_interval_s * 1000;
        writeCursorCache(opts.dataDir, opts.sink.url, next);
    };
    /** Sign the CURRENT local head — the anti-withholding device. */
    const signHead = async (seq, hash) => {
        try {
    ...
    for (;;) {
        ...
        if (localHead.seq === 0) {
            ...
            if (opts.drain === true)
                return result(0);
            if (idleExitMs > 0 && now() - lastActivityAt >= idleExitMs)
                return result(0);
            await sleep(pollMs);
            continue;
        }
        ...
        if (cursor !== undefined && (cursor.chain_id !== chainId || cursor.key !== opts.signer.publicKeyHex)) {
            state = 'refused';
            noteError(`sink answered for chain ${cursor.chain_id.slice(0, 12)}/key ${cursor.key.slice(0, 12)}, ` +
        ...
        if (state !== 'forked' && cursor !== undefined && nextSeq >= 1) {
            const expected = nextSeq === 1 ? GENESIS_HASH : (recordAt(opts.store, nextSeq - 1)?.hash ?? undefined);
            if (expected === undefined) {
                ...
                if (nextSeq - 1 > localHead.seq) {
                    state = 'stalled';
                    noteError(`receiver is at seq ${String(nextSeq - 1)} but the local head is ${String(localHead.seq)}`);
            ...
            if (expected !== cursor.head_hash) {
                state = 'forked';
                noteError(`receiver's hash at seq ${String(nextSeq - 1)} does not match the local chain — ` +
                    'fork or MITM; refusing to ship');
                logOnce('fork-headhash', `[mcp-recorder] sink FORK DETECTED (terminal for this chain): ${lastError ?? ''}`);
            }
        }
        ...
        if (!backlog) {
            ...
            if (opts.drain !== true && heartbeatDue) {
                ...
                if (head !== undefined) {
                    posts++;
                    lastPostAt = now();
                    const outcome = await activeClient.postBatch(activeClient.buildBody({
                        fromSeq: 0,
                        toSeq: 0,
            ...
            if (opts.drain === true)
                return result(localHead.seq);
            if (opts.maxPosts !== undefined && posts >= opts.maxPosts)
                return result(localHead.seq);
            if (idleExitMs > 0 && now() - lastActivityAt >= idleExitMs)
                return result(localHead.seq);
        ...
        switch (outcome.kind) {
            ...
            case 'gap': {
                ...
                if (outcome.cursor !== undefined)
                    ...
                    if (refetched.kind === 'ok' && refetched.cursor !== undefined)
                        applyCursor(refetched.cursor);
                }
                noteError(outcome.detail);
                state = 'retrying';
                break;
            ...
            case 'fork': {
                ...
                noteError(outcome.detail);
                logOnce('fork', '[mcp-recorder] sink FORK DETECTED (terminal for this chain, recording continues locally): ' +
                    outcome.detail);
                break;
            }
            case 'bad_request': {
            ...
            case 'unauthorized': {
                state = 'unauthorized';
                noteError(outcome.detail);
                logOnce('unauthorized', `[mcp-recorder] sink refused this install's credential (${String(outcome.status)}): ${outcome.detail}`);
                break;
            }
            case 'too_large': {
                if (records.length === 1) {
                    ...
                    if (outcome.maxBytes !== undefined)
                        maxBytes = Math.max(1, outcome.maxBytes);
                    noteError(outcome.detail);
                    state = 'retrying';
                }
                break;
        ...
        if (state === 'retrying' || state === 'unauthorized') {
            if (opts.drain === true && outcome.kind === 'unauthorized')
                return result(localHead.seq);
            let delay;
