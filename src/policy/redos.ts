...
interface Atom {
  /** Source text of the atom itself, without its quantifier. */
  text: string;
  ...
  frame?: Frame;
  /**
   * Set when {@link inlinePlainGroups} lifted this atom out of a group it was
   * the whole of: the group source as the author wrote it (`([a-z]+)` for an
   * atom whose own text is `[a-z]+`), so a message names the pattern text.
   */
  written?: string;
}
...
function braceQuantifier(pattern: string, at: number): { text: string; min: number; max: number } | undefined {
...
function repeats(quant: string): boolean {
  if (quant.startsWith('*') || quant.startsWith('+')) return true;
...
function optional(quant: string): boolean {
  if (quant.startsWith('*') || quant.startsWith('?')) return true;
  if (quant.startsWith('+')) return false;
  const parsed = braceQuantifier(quant, 0);
...
function escapeLength(pattern: string, at: number): number {
  if (pattern[at + 1] === 'x' && /^[0-9A-Fa-f]{2}$/.test(pattern.slice(at + 2, at + 4))) return 4;
...
function classEnd(pattern: string, at: number): number {
  let i = at + 1;
  if (pattern[i] === '^') i++;
  if (pattern[i] === ']') i++; // a leading "]" is a literal in RE2 (the subset check rejects it separately)
...
function groupPrefixLength(pattern: string, at: number): number {
  if (pattern[at + 1] !== '?') return 1;
...
function readQuantifier(pattern: string, at: number): string | undefined {
  const ch = pattern[at];
  if (ch === '*' || ch === '+' || ch === '?') {
...
function matcher(text: string): ((ch: string) => boolean) | undefined {
  try {
...
function anchors(repeated: Atom, anchor: Atom): boolean {
  if (anchor.group || anchor.zeroWidth) return false;
  if (anchor.quant !== undefined && optional(anchor.quant)) return false;
  if (repeated.group) return false; // an opaque body: assume it can swallow the anchor
  const repeatedMatches = matcher(repeated.text);
  const anchorMatches = matcher(anchor.text);
  if (repeatedMatches === undefined || anchorMatches === undefined) return false;
  let anchorMatchedSomething = false;
  for (const ch of PROBE_CHARS) {
    if (!anchorMatches(ch)) continue;
    anchorMatchedSomething = true;
    if (repeatedMatches(ch)) return false;
  }
...
function shownText(a: Atom): string {
...
function shapeProblem(frame: Frame, label: string, depth: number): string | undefined {
  ...
  if (repeatedAt === -1) return undefined;
  const repeated = atoms[repeatedAt] as Atom;
 ...
 * the author typed it.
 */
function inlinePlainGroups(branch: readonly Atom[], depth: number): Atom[] {
  const out: Atom[] = [];
  for (const atom of branch) {
    ...
    const inner = inlinePlainGroups(frame.branches[0] as Atom[], depth + 1);
    const consuming = inner.filter((a) => !a.zeroWidth);
    // `([a-z]+)` is one atom wearing parentheses: keep the source for the message.
    if (consuming.length === 1) out.push({ ...(consuming[0] as Atom), written: atom.text });
    else out.push(...inner);
  }
  return out;
}
  ...
  for (let i = 0; i < branch.length; i++) {
    ...
    for (let j = i + 1; j < branch.length; j++) {
      ...
      if (isRepeated(next) && overlap(first, next)) {
        return (
          `regex shape ${JSON.stringify(shownText(first) + shownText(next))} repeats two adjacent atoms that can match ` +
 ...
 */
function startsAnchored(root: Frame): boolean {
  if (root.branches.length !== 1) return false;
  const first = (root.branches[0] as Atom[])[0];
