 ...
 */
export function detectLeg(doc: unknown): PolicyLeg {
  if (typeof doc !== 'object' || doc === null || Array.isArray(doc)) return 'gateway';
  const keys = Object.keys(doc);
  if (keys.includes('version') || keys.includes('mcp') || keys.includes('egress')) return 'gateway';
...
export function smokeGateway(policy: Policy, server: string, tool: string, args: unknown): SmokeResult {
  const argsBytes = Buffer.byteLength(JSON.stringify(args ?? {}), 'utf8');
  const d = evaluateMcp(policy, { server, tool, args: args ?? {}, argsBytes });
  const result: SmokeResult = { leg: 'gateway', tool, server, action: d.action, rule: ruleLabel(d) };
  if (d.reason !== undefined) result.reason = d.reason;
  if (d.failClosed === true) result.fail_closed = true;
  return result;
}

export function smokeHook(policy: CompiledPolicy, toolName: string): SmokeResult {
  const d = evaluatePolicy(policy, toolName);
  const result: SmokeResult = { leg: 'hook', tool: toolName, action: d.decision };
  if (d.reason !== undefined) result.reason = d.reason;
