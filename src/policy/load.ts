 ...
 * decoration.
 */
export function fileTrustFrom(owner: FileOwnership | undefined, me: ProcessIdentity | undefined): PolicyFileTrust {
  if (owner === undefined) return { writableByThisUid: true, detail: 'the policy file could not be stat()ed' };
  if (me === undefined) {
    return {
  ...
  if (me.uid === 0) return { writableByThisUid: true, detail: 'the recorder runs as root, which ignores the mode bits' };
  if ((mode & 0o002) !== 0) return { writableByThisUid: true, detail: `the policy file is world-writable (mode ${mode.toString(8).padStart(4, '0')})` };
...
export function processIdentity(): ProcessIdentity | undefined {
...
export function policyFileTrust(path: string): PolicyFileTrust {
 ...
 */
export function credentialTrustProblems(policy: Policy, trust: PolicyFileTrust): string[] {
  if (!trust.writableByThisUid) return [];
  const out: string[] = [];
  for (const credential of policy.credentials ?? []) {
    if (credential.source?.type !== 'exec') continue;
    out.push(
      `credential ${JSON.stringify(credential.id)} resolves through an \`exec\` source, but ${trust.detail} — ` +
