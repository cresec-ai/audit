...
export function starterPolicyPath(dataDir: string): string {
  return join(dataDir, STARTER_POLICY_FILE);
}

/** `<data-dir>/policy.starter.json` — the hook leg's twin. */
export function starterHookPolicyPath(dataDir: string): string {
 ...
 */
export const STARTER_RULE_COUNTS = { deny: 4, hold: 3 } as const;
 ...
 */
export function materialiseStarterFile(path: string, contents: string): StarterFile {
...
export function materialiseStarterPolicy(dataDir: string): { policy: StarterFile; hook: StarterFile } {
  return {
