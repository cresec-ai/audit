...
export const POLICY_SCHEMA: JsonSchema = {
  ...
  $defs: {
    ...
    anyArgBudget: {
      ...
      additionalProperties: false,
      description:
        'How much of one call\'s arguments match.any_arg may scan before the local engine denies fail-closed. Raising it scans MORE, never less.',
      properties: {
        max_leaves: {
          type: 'integer',
    ...
    mcpMatch: {
      ...
      properties: {
        ...
        any_arg: {
          ...
          maxLength: 2048,
          description:
            'One RE2-compatible regex searched against every STRING LEAF of params.arguments, at any depth and under any key; the rule matches when any one leaf matches. Object keys, numbers and booleans are not scanned. Arguments past the scan budget (256 leaves / 256 KiB) are unevaluable and deny.',
        },
        max_args_bytes: {
          type: 'integer',
          minimum: 0,
