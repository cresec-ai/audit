...
import type { Credential, CredentialUse, EgressRule, McpRule, Policy } from './types.js';
...
export function bundleRoots(policy: Policy): string[] {
  ...
  if (policy.credentials !== undefined) roots.push(CREDENTIALS_ROOT);
  if (policy.egress !== undefined) roots.push(EGRESS_ROOT);
  return roots;
}

/** Canonical write order for bundle files. */
export const BUNDLE_FILE_ORDER: readonly string[] = [MANIFEST_PATH, MCP_REGO_PATH, CREDENTIALS_REGO_PATH, EGRESS_REGO_PATH];
...
interface ModuleSpec<R extends { id: string; action: string; reason?: string }> {
  ...
  /** Comment lines (without the leading "# ") after the header, for a module that needs to say something the others do not. */
...
function renderModule<R extends { id: string; action: string; reason?: string }>(
  ...
  for (const note of spec.notes ?? []) out.push(note === '' ? '#' : `# ${note}`);
  out.push('');
...
/** One declared swap site, flattened out of its credential so it can be a Rego rule. */
...
function credentialRules(credentials: readonly Credential[]): CredentialRule[] {
  const out: CredentialRule[] = [];
  for (const credential of credentials) {
    for (const use of credential.use) {
      const rule: CredentialRule = { id: use.id, action: use.action, credentialId: credential.id, use };
      if (use.reason !== undefined) rule.reason = use.reason;
      out.push(rule);
    }
  }
  return out;
 ...
 */
function credentialRuleBody(rule: CredentialRule): string[] {
  const use = rule.use;
  const lines: string[] = [`input.credential == ${q(rule.credentialId)}`];
 ...
 */
export function renderCredentialsModule(policy: Policy, opts: CompileOptions): string {
  const credentials = policy.credentials;
  if (credentials === undefined) throw new TypeError('policy has no credentials section');
  return renderModule<CredentialRule>(
    {
      pkg: 'cresec.credentials',
      inputShape:
        '{"credential": "...", "server": "...", "tool": "...", "host": "...", "host_source": "argument"|"declared"|"server_name", "path_template": "..."}',
...
export function compileToRego(policy: Policy, opts: CompileOptions): RegoBundle {
  ...
  if (policy.credentials !== undefined) files[CREDENTIALS_REGO_PATH] = renderCredentialsModule(policy, opts);
