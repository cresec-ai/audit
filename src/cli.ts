 ...
 */
async function resolveActor(flags: Flags, policy: LoadedPolicy | undefined, what: string): Promise<LoadedActor | undefined> {
  ...
  try {
    ...
    if (loaded === undefined) {
      if (jwks !== undefined && jwks !== '') err(`${what}: --identity-jwks needs an identity JWT (--identity-jwt PATH, or the policy's identity_jwt_env)`);
      return undefined;
    }
    diag(
      `identity: actor ${loaded.actor.user.email} via ${loaded.actor.tool.name}@${loaded.actor.tool.version}` +
        ` (run_as ${loaded.actor.run_as}; ${loaded.verified ? 'signature verified' : 'NOT verified: decoded only'})`,
    );
    return loaded;
  } catch (cause) {
    if (cause instanceof IdentityJwtError) return err(`${what}: ${cause.message}`);
    throw cause;
  }
}
...
async function resolveGateway(
  ...
  const gateway: GatewayOptions = { policy, holdStore: new HoldStore(config.dataDir) };
...
async function cmdHook(flags: Flags, positionals: string[]): Promise<void> {
  ...
  try {
    ...
    let actor: ActorStamp | undefined;
    try {
      const jwtPath = asStr(flags['identity-jwt']);
      const jwks = asStr(flags['identity-jwks']);
      const loaded =
        jwtPath === undefined || jwtPath === ''
          ? undefined
