...
export function isRpcId(v: unknown): v is string | number {
...
export function isToolsCallRequest(msg: unknown): msg is Record<string, unknown> & { id: string | number } {
...
export function isInvalidIdToolsCall(msg: unknown): msg is Record<string, unknown> {
  return isPlainObject(msg) && msg['method'] === 'tools/call' && 'id' in msg && !isRpcId(msg['id']);
}

/** A `tools/call` NOTIFICATION: no `id` property at all. */
export function isToolsCallNotification(msg: unknown): msg is Record<string, unknown> {
...
export function toolsCallParts(msg: Record<string, unknown>): { params: Record<string, unknown>; name: string } {
...
export function isRpcResponse(msg: unknown): msg is Record<string, unknown> & { id: string | number } {
...
export function looksLikeToolResult(msg: Record<string, unknown>): boolean {
  const result = msg['result'];
  return isPlainObject(result) && Array.isArray(result['content']);
}

/** `{"jsonrpc":"2.0","id":<id|null>,"error":{"code":-32600,...}}`. */
export function invalidRequestResponse(id: string | number | null, message: string): Record<string, unknown> {
 ...
 */
export function rewriteSseEvent(raw: Buffer, data: string): Buffer {
  ...
  if (!dataWritten) out.unshift('data: ' + data);
...
export class SseEventSplitter {
  ...
  constructor(private readonly maxEventBytes: number = MAX_GATEWAY_BODY) {}

  /** Feed a chunk; get back every event completed by it. */
  push(chunk: Buffer): Array<SseEvent | { raw: Buffer; oversized: true }> {
    const out: Array<SseEvent | { raw: Buffer; oversized: true }> = [];
    if (this.overflowed) {
  ...
  /** The unterminated tail at end of stream, if any, as one final event. */
    ...
    if (this.pendingLen === 0) return undefined;
...
function findEventEnd(buf: Buffer, from: number): number {
  let i = from;
  while (i < buf.length) {
    ...
    if (lineEnd === lineStart) return nl + 1;
    i = nl + 1;
  }
  return -1;
}

function eventData(raw: Buffer): string | undefined {
