 ...
 */
function isNullIdToolsCall(msg: unknown): msg is Record<string, unknown> {
...
class JsonSpanScanner {
   ...
   */
  member(key: string): [number, number] | undefined {
    ...
    for (;;) {
      ...
      if (raw === wanted || (raw.includes('\\') && (JSON.parse(raw) as string) === key)) {
        found = [start, this.i];
      }
      this.ws();
      const c = this.s[this.i];
  ...
  element(index: number): [number, number] | undefined {
    this.ws();
    if (this.s[this.i] !== '[') this.fail();
    this.i++;
    let found: [number, number] | undefined;
    ...
    for (;;) {
      ...
      const start = this.i;
      this.skipValue();
      if (at === index) found = [start, this.i];
      at++;
      this.ws();
      const c = this.s[this.i];
      if (c === ',') {
...
function jsonValueSpan(text: string, path: JsonPath): [number, number] | undefined {
  try {
    const sc = new JsonSpanScanner(text);
    let span: [number, number] = [0, text.length];
...
function collectDivergences(prev: unknown, next: unknown, path: JsonPath, out: JsonPath[]): boolean {
  ...
  if (out.length >= MAX_LINE_EDITS) return false;
...
export async function runStdioProxy(opts: StdioProxyOpts): Promise<number> {
  ...
  if (gateway !== undefined) {
    ...
    const refuseReusedId = (
      ...
      if (holds.has(key)) return refuseDuplicateId(msg, 'held');
      if (pending.has(key) || claimed?.has(key) === true) return refuseDuplicateId(msg, 'pending');
      return undefined;
    };
     ...
     */
    const refuseNullIdToolsCall = (
      msg: Record<string, unknown>,
      line: ScannedLine,
      inBatch = false,
    ): Record<string, unknown> => {
      diag(
        `gateway: refused a tools/call with a null id${inBatch ? ' inside a JSON-RPC batch' : ''}` +
