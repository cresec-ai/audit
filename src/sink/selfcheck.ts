...
import type { EvidenceStore, VerifyProblem } from '../types.js';
...
export function newSelfCheck(): ChainSelfCheck {
...
function describe(problem: VerifyProblem): string {
 ...
 * in one pass.
 */
export async function advanceSelfCheck(
  store: EvidenceStore,
  self: ChainSelfCheck,
  toSeq: number,
  batch: readonly ChainRecord[],
  opts: SelfCheckOpts = {},
): Promise<string | undefined> {
  if (self.failure !== undefined) return self.failure;
  if (toSeq <= self.through) return undefined;
   ...
   */
  const newestPerWindow = new Map<number, { seq: number; sigs: HeadSignature[] }>();
  for (const sig of store.signatures()) {
    if (sig.seq <= base || sig.seq > toSeq) continue;
  ...
  function* source(): Generator<ChainRecord> {
    if (base + 1 <= streamTo) yield* store.iterate({ fromSeq: base + 1, toSeq: streamTo });
    for (const record of batch) {
      if (record.seq > base && record.seq <= toSeq) yield record;
    }
  }

  let window: ChainRecord[] = [];
  let windowIndex = 0;
  const closeWindow = async (): Promise<string | undefined> => {
    const first = window[0]!.seq;
    const last = window[window.length - 1]!.seq;
    if (first !== self.through + 1) {
      return fail(
        `seq_gap at seq ${String(self.through + 1)}: the local store no longer holds it, ` +
    ...
    if (hard !== undefined) return fail(describe(hard));
    if (result.head.seq !== last) {
      ...
      return fail(
        `malformed_record at seq ${String(result.head.seq + 1)}: the record could not be ` +
  ...
  if (self.through < toSeq) {
    return fail(
      `seq_gap at seq ${String(self.through + 1)}: the local store stops at seq ` +
