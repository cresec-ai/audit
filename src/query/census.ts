...
function isHookDeny(ev: AnyEvent): boolean {
  if (ev.kind !== 'tool_call' || ev.phase !== 'pre') return false;
  return ev.error?.type === 'policy_denied';
}

/** Rows sorted by server, then tool (plain code-unit order, stable run to run). */
export function toolCensus(records: Iterable<ChainRecord>): ToolCensusRow[] {
  const rows = new Map<string, { row: ToolCensusRow; sessions: Set<string> }>();
  for (const record of records) {
    ...
    if (ev.timestamp < row.first_seen) row.first_seen = ev.timestamp;
    if (ev.timestamp > row.last_seen) row.last_seen = ev.timestamp;
    ...
    if (ev.is_error === true) row.errors += 1;
    if (ev.gateway?.decision === 'deny' || isHookDeny(ev)) row.denied += 1;
    if (ev.gateway?.decision === 'hold') row.held += 1;
  }
  return [...rows.values()]
    .map((e) => e.row)
