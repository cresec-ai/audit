...
export interface DoctorReport {
  ...
  config?: string;
  policy?: { path: string; hash: string; rules: number };
  hook_policy?: { path: string; deny_rules: number };
  checks: DoctorCheck[];
...
interface StdioResult {
  tools?: string[];
  /** The text of the first `tools/call` result, when one was requested. */
  callText?: string;
  callIsError?: boolean;
  error?: string;
}
...
async function stdioSession(
  ...
  opts: {
    ...
    /**
     * Close stdin and WAIT for the child to exit before resolving, instead of
     * killing it.
     *
     * The probe needs this and discovery does not. Recording is fail-open and
     * therefore asynchronous: the gateway's refusal reaches the client before
     * the `policy_decision` reaches the store. Killing the recorder the
     * instant the refusal arrives loses the event, and the probe then reports
     * the exact absence it exists to detect — a deny that fired, with a chain
     * that holds nothing.
     */
    graceful?: boolean;
  },
): Promise<StdioResult> {
  return new Promise<StdioResult>((resolveResult) => {
    ...
    const out: StdioResult = {};
    let buf = '';
    let settled = false;
    let resolved = false;
    const done = (extra?: Partial<StdioResult>): void => {
      if (resolved) return;
      resolved = true;
      try {
        child.kill('SIGKILL');
    ...
    const finish = (extra?: Partial<StdioResult>): void => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
    ...
    child.on('exit', (code, signal) =>
      finish(out.tools === undefined ? { error: `exited with ${signal ?? code} before answering` } : {}),
    );
    ...
    child.stdout?.on('data', (chunk: string) => {
      ...
      while ((nl = buf.indexOf('\n')) !== -1) {
        ...
        if (msg.id === 3) {
          const result = msg.result as { isError?: unknown; content?: Array<{ text?: unknown }> } | undefined;
 ...
 */
export function connectorsFromConfigs(env: NodeJS.ProcessEnv): ConnectorEntry[] {
  ...
  for (const path of candidateConfigPaths(env, simpleGlob)) {
    ...
    for (const [configKey, raw] of Object.entries(servers as Record<string, unknown>)) {
      if (typeof raw !== 'object' || raw === null) continue;
      const entry = raw as { tools?: unknown; url?: unknown };
...
function ok(id: string, title: string, summary: string, detail: string[] = []): DoctorCheck {
  return { id, title, status: 'OK', summary, detail };
}
function fail(id: string, title: string, summary: string, detail: string[]): DoctorCheck {
  return { id, title, status: 'FAIL', summary, detail };
}
function incomplete(id: string, title: string, summary: string, detail: string[]): DoctorCheck {
 ...
 */
export function entryDataDir(entry: ServerEntry, fallback: string): string {
  ...
  if (eq !== undefined) return eq.slice('--data-dir='.length);
 ...
 */
export function readWiring(servers: McpServersMap, cliPath: string, dataDir: string): WiringEntry[] {
  return Object.entries(servers).map(([name, entry]) => {
    if (typeof entry !== 'object' || entry === null) {
      return { name, entry: {} as ServerEntry, wrapped: false, problem: 'not a JSON object' };
    }
    ...
    const out: WiringEntry = { name, entry, wrapped: true };
    if (policyPath !== undefined) out.policyPath = policyPath;
    if (inner !== undefined) out.inner = inner;
    if (policyPath === undefined) out.problem = 'wrapped, but with no --policy — recording only, nothing is enforced';
...
export function readHookWiring(settingsPath: string, cliPath: string): HookWiring {
  if (!existsSync(settingsPath)) return { installed: false, problem: `no hook settings at ${settingsPath}` };
  let root: Record<string, unknown>;
  try {
    root = JSON.parse(stripBom(readFileSync(settingsPath, 'utf8'))) as Record<string, unknown>;
  } catch (cause) {
    return { installed: false, problem: `${settingsPath} is not valid JSON: ${cause instanceof Error ? cause.message : String(cause)}` };
  }
  const hooks = root['hooks'];
  const pre = typeof hooks === 'object' && hooks !== null ? (hooks as Record<string, unknown>)['PreToolUse'] : undefined;
  if (!Array.isArray(pre)) return { installed: false, problem: 'no PreToolUse hook is installed' };
  for (const group of pre) {
    ...
    for (const h of entries) {
      const command = typeof h === 'object' && h !== null ? (h as { command?: unknown }).command : undefined;
      if (typeof command !== 'string') continue;
      if (!command.includes('mcp-recorder') && !command.includes(cliPath) && !command.includes('cli.js')) continue;
      const m = /--policy[= ]("([^"]+)"|'([^']+)'|(\S+))/.exec(command);
      const policyPath = m?.[2] ?? m?.[3] ?? m?.[4];
      const out: HookWiring = { installed: true };
      if (typeof g.matcher === 'string') out.matcher = g.matcher;
      if (policyPath !== undefined) out.policyPath = policyPath.replace(/^["']|["']$/g, '');
      else out.problem = 'the PreToolUse hook is installed but carries no --policy — it records, it does not enforce';
      return out;
    }
  }
...
export function checkSpellings(
  hookPolicy: CompiledPolicy,
  tools: Array<{ configKey: string; tool: string; host?: string; friendly?: string }>,
): SpellingVerdict[] {
  const out: SpellingVerdict[] = [];
  hookPolicy.deny.forEach((rule, ruleIndex) => {
    for (const t of tools) {
      const spellings = toolSpellings(t);
      const matched: ToolSpelling[] = [];
      const missed: ToolSpelling[] = [];
      for (const s of spellings) {
        ...
        if (new RegExp(rule.tool.source, rule.tool.flags.replace(/[gy]/g, '')).test(s.name)) matched.push(s);
        else missed.push(s);
      }
...
export function coverage(
  ...
  hookPolicy: CompiledPolicy | null,
  stdio: Array<{ server: string; tools: string[] }>,
 ...
 */
function probeArgumentCandidates(policyPath: string, dataDir: string): Array<Record<string, unknown>> {
  return [
    {},
    { path: policyPath },
    { path: dataDir },
...
export function planProbe(
  ...
  policyPath: string,
  dataDir: string,
  stdio: Array<{ server: string; tools: string[]; inner?: ServerEntry }>,
): ProbePlan | undefined {
  for (const s of stdio) {
    if (s.inner === undefined || typeof s.inner.command !== 'string') continue;
    for (const tool of s.tools) {
...
function decisionsIn(dataDir: string): number {
  try {
    ...
    try {
      let n = 0;
      for (const rec of store.iterate()) if (rec.event.kind === 'policy_decision') n++;
      return n;
    } finally {
      store.close();
    }
 ...
 * measured by.
 */
export function probeVerdict(plan: ProbePlan, obs: ProbeObservation): DoctorCheck {
...
async function waitForDecision(dataDir: string, budgetMs: number): Promise<number> {
 ...
 */
export function liveSessionConfigPaths(env: NodeJS.ProcessEnv): string[] {
 ...
 */
export function sessionSnapshotIsStale(settingsPath: string, livePaths: readonly string[]): boolean {
...
export async function runDoctor(opts: DoctorOptions): Promise<DoctorReport> {
  ...
  try {
    const parsed = JSON.parse(stripBom(readFileSync(opts.configPath, 'utf8'))) as Record<string, unknown>;
    const raw = parsed['mcpServers'];
    if (typeof raw === 'object' && raw !== null && !Array.isArray(raw)) servers = raw as McpServersMap;
  } catch (cause) {
  ...
  /* --- C1 wiring ------------------------------------------------------- */
  {
    ...
    if (killSwitch) {
      detail.push(
        `  environment ${ENV.DISABLE}=1 is set — the kill switch is on; with it set, neither recording nor`,
        '              the gateway runs, whatever the config says',
      );
    }
  ...
  /* --- C2 hook wiring -------------------------------------------------- */
  let hookPolicy: CompiledPolicy | null = null;
  let hookPolicyPath: string | undefined;
  {
    if (opts.settingsPath === undefined) {
      ...
      checks.push(
        incomplete(
          ...
          noHookMechanism
            ...
            : 'no Claude Code settings file was given, so the hook leg was not checked',
          noHookMechanism
            ? [
      ...
      if (hw.policyPath !== undefined) {
        ...
        if (loaded.warning !== undefined) hw.problem = loaded.warning;
        if (hookPolicy !== null) report.hook_policy = { path: hw.policyPath, deny_rules: hookPolicy.deny.length };
      }
  ...
  const stdioServers: Array<{ server: string; tools: string[]; inner?: ServerEntry; error?: string }> = [];
  for (const w of wiring) {
    if (w.inner === undefined || typeof w.inner.command !== 'string') continue;
    const r = await stdioSession(w.inner.command, (w.inner.args ?? []) as string[], {
      timeoutMs,
  ...
  for (const c of connectors) report.tools[c.configKey] = c.tools;

  /* --- C3 name spellings ----------------------------------------------- */
  {
    const real = connectors.flatMap((c) =>
      c.tools.map((tool) => ({ configKey: c.configKey, tool, ...(c.host !== undefined ? { host: c.host } : {}) })),
    );
    ...
    if (hookPolicy === null) {
      checks.push(
        incomplete('C3', 'name spellings', 'no hook policy is loaded, so no deny rule could be checked', [
      ...
      const behavioural = new Map<number, SpellingVerdict>();
      for (const v of verdicts) if (v.missed.length > 0 && !behavioural.has(v.ruleIndex)) behavioural.set(v.ruleIndex, v);
      const syntactic = new Map<number, string>();
      hookPolicy.deny.forEach((rule, i) => {
        const segment = anchoredSegment(rule.tool.source);
        if (segment !== undefined) syntactic.set(i, segment);
      });
      ...
      if (anchoredRules.length > 0) {
        ...
        for (const i of anchoredRules) {
          ...
          if (v !== undefined) {
            for (const m of v.matched) detail.push(`    matches      ${m.name}`);
            for (const m of v.missed) detail.push(`    MISSES       ${m.name}   (${m.origin})`);
          }
          detail.push('');
        }
        detail.push(
          '  The <server> segment is chosen by the client, not by you, and it has differed',
          '  between two sessions a day apart and between two surfaces of one machine at the',
          '  same time. A rule anchored to one spelling stops matching when it changes,',
          '  silently, with no error anywhere: cloud dogfood 4 ended with two live connector',
          '  calls against a real workspace and a 62-event signed bundle holding ZERO policy',
          '  decisions, for exactly this reason.',
          '',
          '  Fix: leave the segment open —  ^mcp__.*__<tool>$',
        );
        checks.push(
          fail('C3', 'name spellings', `${anchoredRules.length} of ${hookPolicy.deny.length} deny rules are anchored to a server segment`, detail),
        );
        ...
        checks.push(
          incomplete(
            'C3',
            'name spellings',
            `${hookPolicy.deny.length} deny rules are segment-open, but no live MCP config was found`,
            [
        ...
        checks.push(
          ok(
            'C3',
            'name spellings',
            `${hookPolicy.deny.length} deny rules x ${spellingsPerTool} spellings x ${tools.length} connector tools — all match`,
  ...
  /* --- C4 coverage ------------------------------------------------------ */
  {
    ...
    if (rows.length === 0) {
      checks.push(
        incomplete('C4', 'coverage', `${unenumerated.length || 'all'} of ${stdioServers.length} servers could not be enumerated — no tools were discovered at all`, [
      ...
      checks.push(
        fail('C4', 'coverage', `${rows.length} tools discovered; 0 denied, 0 held, ${rows.length} allowed`, [
      ...
      checks.push(
        incomplete('C4', 'coverage', `${unenumerated.length} of ${stdioServers.length} servers could not be enumerated`, [
          ...unenumerated.map((s) => `  ${s.server}  ${s.error ?? ''} — no tool list, so this server's coverage is UNKNOWN.`),
          '  It is not "fine"; it is unchecked.',
          ...detail,
        ]),
      ...
      checks.push(
        ok(
          'C4',
          'coverage',
          `${rows.length} tools discovered (${stdioServers.length} servers, ${connectors.length} connectors)`,
          detail,
        ),
      );
    }
  ...
  /* --- C5 probe --------------------------------------------------------- */
  {
    if (!opts.probe) {
      ...
      if (plan === undefined) {
        checks.push(
          incomplete('C5', 'probe', 'no discovered tool is denied by this policy on a call with no side effects', [
        ...
        try {
          const r = await stdioSession(
            ...
            [
              ...
              policyPath,
              '--',
              plan.inner.command as string,
              ...((plan.inner.args ?? []) as string[]),
            ],
  ...
  /* --- C6 chain --------------------------------------------------------- */
  {
    try {
      ...
      try {
        ...
        let decisions = 0;
        for (const rec of store.iterate()) if (rec.event.kind === 'policy_decision') decisions++;
      ...
      checks.push(
        incomplete('C6', 'chain', `the evidence directory could not be read: ${cause instanceof Error ? cause.message : String(cause)}`, [
          `  ${opts.dataDir}`,
        ]),
      );
    }
...
export function renderDoctor(report: DoctorReport): string[] {
  const lines: string[] = [];
  if (report.config !== undefined) lines.push(`config: ${report.config}`);
  ...
  for (const c of report.checks) {
    ...
    if (c.detail.length > 0) lines.push('');
  }
 ...
 */
export function doctorVerdictLines(report: DoctorReport): string[] {
  const okCount = report.checks.filter((c) => c.status === 'OK').length;
  const failCount = report.checks.filter((c) => c.status === 'FAIL').length;
  const incCount = report.checks.filter((c) => c.status === 'INCOMPLETE').length;
