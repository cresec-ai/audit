 ...
 */
export function templatePath(pathTemplate: string): string {
  if (pathTemplate === '' || !pathTemplate.startsWith('/')) return pathTemplate;
  return pathTemplate
    .split('/')
    .map((seg) =>
      seg !== '' && (UUID_SEGMENT.test(seg) || NUMERIC_SEGMENT.test(seg) || HEX_SEGMENT.test(seg) || OPAQUE_SEGMENT.test(seg)) ? '{id}' : seg,
    )
    .join('/');
}

interface HashedCredential {
  syntheticHash: Buffer;
  connector: string;
  sites: Record<string, UserTokenSite>;
}

/** An outage of the control plane as THIS process observed it. */
interface Outage {
  /** When the first failed request was observed. */
  since: number;
  /** No request is sent before this instant. */
  retryAt: number;
  /** A probe is in flight: every other call is refused without asking. */
  probing: boolean;
  /** Calls refused without a request while the outage held. */
  refused: number;
}
...
export class RemoteBroker implements Broker {
  readonly #opts: RemoteBrokerOptions;
  readonly #warn: (line: string) => void;
  readonly #now: () => number;
  #outage: Outage | undefined;
  /** Per-credential synthetic hashes, peppered per process, for the user-token path. */
  readonly #credentials = new Map<string, HashedCredential>();
  readonly #pepper: Buffer;

  constructor(opts: RemoteBrokerOptions) {
    if (!opts.baseUrl) throw new Error('mcp-recorder: RemoteBroker needs a baseUrl');
    ...
    if (opts.userToken !== undefined) {
      ...
      if (ut.tenant === '') throw new Error('mcp-recorder: RemoteBroker user-token mode needs a tenant');
      if (!UUID.test(ut.userId)) throw new Error('mcp-recorder: RemoteBroker user-token mode needs a uuid user id');
      if (ut.tool.id === '' || ut.tool.version === '') throw new Error('mcp-recorder: RemoteBroker user-token mode needs a tool id and version');
      for (const [id, cred] of Object.entries(ut.credentials)) {
        if (cred.synthetic === '') throw new Error(`mcp-recorder: credential "${id}" has no synthetic bound`);
  ...
  async exchange(req: BrokerExchangeRequest, hint?: BrokerExchangeHint): Promise<BrokerExchangeResponse> {
    if (this.#opts.userToken !== undefined) return this.#userToken(req, hint);
    return this.#brokerExchange(req);
  }

  /* ----------------------- /v1/broker/user-token ---------------------- */

  async #userToken(req: BrokerExchangeRequest, hint: BrokerExchangeHint | undefined): Promise<BrokerExchangeResponse> {
    const ut = this.#opts.userToken as UserTokenConfig;
    ...
    if (cred === undefined || hint === undefined) return denyResponse(newDecisionId(), 'unknown_synthetic');
    ...
    if (site === undefined) return denyResponse(newDecisionId(), 'denied_by_policy');
    if (!this.#admit()) return denyResponse(newDecisionId(), 'control_plane_unavailable');
    let res: BrokerExchangeResponse;
    try {
    ...
    if (res.denied === true && res.deny_reason === 'control_plane_unavailable') this.#observeDown();
    else this.#observeUp();
    return res;
  }
  ...
  #observeDown(): void {
    ...
    if (o === undefined) {
      ...
      this.#warn(
        `broker: control plane unavailable (observed by this process at ${new Date(now).toISOString()}); ` +
  ...
  #observeUp(): void {
    ...
    const now = this.#now();
    this.#warn(
      `broker: control plane reachable again at ${new Date(now).toISOString()} — unavailable for ` +
        `${String(now - o.since)} ms as observed by this process, ${String(o.refused)} call(s) refused without asking it`,
    );
  }

  async #userTokenRequest(
    ut: UserTokenConfig,
    cred: HashedCredential,
    site: UserTokenSite,
    ...
    if (this.#opts.authToken !== undefined) headers.authorization = `Bearer ${this.#opts.authToken}`;
    ...
    const obj = typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) ? (parsed as Record<string, unknown>) : undefined;
    const decisionId = obj !== undefined && typeof obj.decision_id === 'string' && UUID.test(obj.decision_id) ? obj.decision_id : newDecisionId();
    const reasonOf = (fallback: string): string => {
  ...
  async #brokerExchange(req: BrokerExchangeRequest): Promise<BrokerExchangeResponse> {
    ...
    const body = JSON.stringify({
      ...
      request: req.request,
    } satisfies BrokerExchangeRequest);
    ...
    if (ttl === 0) return denyResponse(decisionId, 'broker_error');
  ...
  async #fetch(req: {
    ...
    if (this.#opts.fetch !== undefined) return await this.#opts.fetch(req);
