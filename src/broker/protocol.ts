...
export interface BrokerExchangeRequest {
  ...
  request: {
    method: string; // MCP: the JSON-RPC method, e.g. "tools/call"
    host: string; // MCP: the resolved server origin or server name
    path_template: string; // MCP: the tool name, e.g. "create_issue"
    user_agent?: string; // MCP: client name/version from initialize
    src_ip?: string;
  };
}
...
export type DenyReason =
  ...
  | 'denied_by_policy'
  | 'vault_missing_token'
  // ── local: the destination was not authorised ───────────────────────
  | 'method_not_permitted'
  | 'host_not_permitted'
  | 'path_not_permitted'
  | 'no_host_constraint'
  // ── local: the credential could not be resolved (fail CLOSED) ───────
  | 'source_timeout'
  | 'source_unhealthy'
  | 'source_unavailable'
  | 'source_empty'
  ...
  | 'source_exec_failed'
  | 'source_untrusted_config'
  | 'github_app_rejected'
  | 'aws_sts_rejected'
  | 'vault_unreachable'
  | 'clickup_token_rejected'
 ...
 */
export function isSyntheticShaped(value: string): boolean {
  if (!value.startsWith(SYNTHETIC_PREFIX)) return false;
  return /^[A-Za-z0-9_-]{16,}$/.test(value.slice(SYNTHETIC_PREFIX.length));
}

/** HMAC-SHA256(value, pepper) — NHI's `hashSynthetic`. */
export function hashSynthetic(value: string, pepper: Buffer): Buffer {
 ...
 */
export function syntheticHashesEqual(a: Buffer, b: Buffer): boolean {
  if (a.length !== b.length) return false;
  return timingSafeEqual(a, b);
}

/** A fresh local pepper. 32 bytes, the size NHI's per-tenant pepper is. */
export function mintPepper(): Buffer {
  return randomBytes(32);
}

/** A denial in NHI's exact shape: no token, zero TTL, a code, a decision id. */
export function denyResponse(decisionId: string, reason: DenyReason): BrokerExchangeResponse {
