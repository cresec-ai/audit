...
import type { EvidenceStore, SessionSummary, VerifyResult } from '../types.js';
...
function shortHex(value: string, n = 16): string {
...
function renderTree(value: Scrubbed, indent = 0): string {
  if (isRedactedRef(value)) return lockChip(value);
...
function timeTag(timestamp: string): string {
  return `<time datetime="${escapeHtml(timestamp)}">${escapeHtml(timestamp)}</time>`;
}

function credentialChips(fingerprints: CredentialFingerprint[] | undefined): string {
  if (fingerprints === undefined || fingerprints.length === 0) return 'none fingerprinted';
  const chips = fingerprints
    .map(
      (cf) =>
        `<span class="lock" data-ref="${escapeHtml(cf.ref)}" title="${escapeHtml(cf.ref)}">` +
...
function renderSessionStart(seq: number, e: SessionStartEvent): string {
  const id = e.identity;
  const who = [
...
function boundarySummary(boundary: GatewayOutcome['boundary']): string {
  ...
  if (secrets > 0) bits.push(`${verb} ${num(boundary.secrets_found)} secret${secrets === 1 ? '' : 's'}`);
  if (injections > 0) {
    bits.push(`${verb} ${num(boundary.injection_found)} injection marker${injections === 1 ? '' : 's'}`);
  }
  if (bits.length === 0) return 'scanned clean';
  return bits.join(' · ');
}
...
function renderToolCall(seq: number, e: ToolCallEvent): string {
  ...
  if (e.error !== undefined) {
    ...
    if (e.error.message_ref !== undefined) {
      bits.push(
        `message <code class="rh" data-ref="${escapeHtml(e.error.message_ref)}" title="${escapeHtml(
          e.error.message_ref,
        )}">${escapeHtml(shortRef(e.error.message_ref))}</code>`,
      );
    }
...
function renderSessionEnd(seq: number, e: SessionEndEvent): string {
  const exit =
...
function integrityBanner(verify: VerifyResult): string {
  if (verify.ok) {
    const sig = verify.verified_signature;
    const signed =
      sig !== undefined
        ? `head signed <code title="${escapeHtml(sig.public_key)}">${escapeHtml(
...
function sessionPicker(sessions: SessionSummary[]): string {
  if (sessions.length === 0) return '<p class="dim">No sessions recorded.</p>';
  const rows = sessions
    .map((s) => {
...
function sessionTimeline(sessionId: string, records: ChainRecord[]): string {
 ...
 */
export function renderTimelineHtml(store: EvidenceStore, opts: RenderOpts = {}): string {
  ...
  const bySession = new Map<string, ChainRecord[]>();
  for (const record of records) {
    const list = bySession.get(record.event.session_id);
    if (list === undefined) bySession.set(record.event.session_id, [record]);
    else list.push(record);
  }

  const visibleSessions =
    selected !== undefined ? allSessions.filter((s) => s.session_id === selected) : allSessions;
  ...
  if (selected !== undefined && records.length === 0) {
    timelines.push(
      `<p class="dim">No events recorded for session <code>${escapeHtml(selected)}</code>.</p>`,
    );
  }
